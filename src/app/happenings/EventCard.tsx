...
/** `muted` is the past-events treatment: no tag, no price, dimmed, still. */
export function EventCard({ event, muted = false }: { event: EventSummary; muted?: boolean }) {
  ...
  return (
    <Card
      ...
      kicker={muted ? `${month} ${day}` : `${weekday}, ${month} ${day} · ${time}`}
      title={event.title}
      meta={event.location ?? undefined}
      price={muted ? undefined : priceLabel(event.priceCents, event.currency)}
    />
  );
}
